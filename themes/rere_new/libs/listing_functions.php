        ...
        if (is_array($plan_title_array)){        
            foreach ($plan_title_array as $key=> $plan_name) {
                ...
                if ( isset($plan_price_array[$key]) && $plan_price_array[$key]!=''){
                    ...
                    if ($plan_price != 0) {
                        $plan_price = number_format($plan_price);
                        if ($where_currency == 'before') {
                            $plan_price = $currency . ' ' . $plan_price;
                        } else {
                            $plan_price = $plan_price . ' ' . $currency;
                        }           
                    }else{
                        $plan_price='';
                    }
                ...
                <div class="front_plan_row">
                    ...
                    <div class="floor_details">';
                        if($plan_price!=''){
                            print  __('price: ','wpestate').' '.$plan_price;
                        }
                        print'</div> 
                </div>
...
function estate_listing_address($post_id){
    ...
    if ($property_county != ''){
        $return_string.= '<div class="listing_detail col-md-4"><strong>'.__('County','wpestate').':</strong> ' . $property_county . '</div>'; 
    }
...
function estate_listing_details($post_id){
    ...
    if ($price !='' ){ 
        $return_string.='<div class="listing_detail col-md-4"><strong>'.__('Price','wpestate'). ':</strong> '.$price.' '.$price_label.'</div>';
    }
